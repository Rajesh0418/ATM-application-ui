...
<body style="background-image: url('<%= request.getContextPath() %>/pictures/atm_bg.jpeg'); background-size: cover; background-position: center;"  
    ...
    <div class="absolute top-4 left-4 flex items-center gap-3">
     ...
     <div class="bg-white/30 backdrop-blur-md p-6 sm:p-10 rounded-xl shadow-2xl w-full max-w-2xl mt-20">
        ...
        <form action="<%= request.getContextPath() %>/transactions/deposit" method="post" class="space-y-4">
            <div>
                <label class="block font-semibold">Account Holder Name :</label>
                <input type="text" name="accountholdername" value="<%= user.getAccountHolderName() %>" class="w-full mt-1 p-2 border rounded-md" placeholder="Enter Account Holder Name" required>
            </div>

            <div>
                <label class="block font-semibold">Account Number :</label>
                <input type="text" name="accountnumber"  value="<%= user.getAccountNumber() %>"class="w-full mt-1 p-2 border rounded-md" placeholder="Enter Account Number" required>
            </div>

			<div>
                <label class="block font-semibold">IFSC Code : </label>
                <input type="text" name="ifsccode" value="<%= user.getIFSCCode() %>" class="w-full mt-1 p-2 border rounded-md" placeholder="Enter IFSC Code" required>
            </div>
